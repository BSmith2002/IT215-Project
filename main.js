// Converted to ES module, imports controls, items, enemies modules.
...
(function(){
  ...
  // initialize controls (this registers listeners and exposes `input`)
  const state = {
  ...
  // wire state.newRun reference so controls can start games with Enter
  ...
  function giveItemWrapper(p, item){
    // wrapper so items module gets state & player
  ...
  function shootPlayer(p){
    ...
    for(let i=0;i<count;i++){
      ...
      if(types.laser){
        // beam: add a Set to track which enemies it already damaged
        state.beams.push({x:p.x,y:p.y, angle:ang, life:120, pierce:true, damage:1+p.mods.damage, hits:new Set()});
      } else {
        // bullets also track hits so a piercing bullet only damages a given enemy once
  ...
  function update(dt){
    ...
    // enemies (movement + behavior) - mostly unchanged and uses onEnemyDie from enemies.js
    for(let i=state.entities.length-1;i>=0;i--){ const e = state.entities[i]; e.t += dt;
      ...
      else if(e.etype==='bomber'){ e.fuse+=dt; e.x += (dx/m)*e.speed; e.y += (dy/m)*e.speed; if(Math.hypot(dx,dy)<35){ explode(state,e.x,e.y); onEnemyDie(state, e); continue; } }
    ...
    for(let i=0;i<state.entities.length;i++){
      for(let j=i+1;j<state.entities.length;j++){
        ...
        const rx=b.x-a.x, ry=b.y-a.y; const d=Math.hypot(rx,ry)||0.0001; const min=a.r+b.r+2;
        if(d<min){ const push=(min-d)/2; const nx=rx/d, ny=ry/d; a.x-=nx*push; a.y-=ny*push; b.x+=nx*push; b.y+=ny*push; }
      }
    }

    // enemy collisions with bullets & beams
    for(let i=state.entities.length-1;i>=0;i--){ const e=state.entities[i];
  // bullets -> enemy collisions
    for(let j=state.bullets.length-1;j>=0;j--){
      const b=state.bullets[j];
      if(b.hits && b.hits.has(e)) continue; // already damaged this enemy
      if(dist2(b.x,b.y,e.x,e.y) < (b.r+e.r)*(b.r+e.r)){
        // mark that this projectile hit this enemy
        if(b.hits) b.hits.add(e);
        e.hp -= (b.dmg||1);
        if(!b.pierce){ state.bullets.splice(j,1); }
        if(b.explosive){ explode(state,b.x,b.y); }
        if(e.hp<=0){
          // mark dead flag so other code won't double-process
          e._dead = true;
          onEnemyDie(state, e);
          break;
        }
    ...
    for(const bm of state.beams){
      if(bm.hits && bm.hits.has(e)) continue; // beam already damaged this enemy
    ...
    // After enemy movement code inside the per-entity loop:
  ...
  for(const pk of state.pickups){
    if(pk.type==='heart'){ cx.save(); cx.translate(pk.x, pk.y); cx.fillStyle = '#ff6b81'; cx.beginPath(); const r=8; cx.moveTo(0, r/2); cx.bezierCurveTo(r, -r/2, r*1.2, r, 0, r*1.4); cx.bezierCurveTo(-r*1.2, r, -r, -r/2, 0, r/2); cx.fill(); cx.restore(); }
    else if(pk.type==='item'){ cx.save(); cx.translate(pk.x,pk.y); cx.fillStyle='#f5d76e'; cx.beginPath(); cx.arc(0,0,10,0,TWO_PI); cx.fill(); cx.fillStyle='#0b0f15'; cx.font='bold 12px system-ui'; cx.textAlign='center'; cx.fillText('★',0,4); cx.restore(); }
    else if(pk.type==='stairs'){ cx.save(); cx.translate(pk.x,pk.y); cx.fillStyle='#7de38b'; cx.beginPath(); cx.moveTo(-10,10); cx.lineTo(0,-10); cx.lineTo(10,10); cx.closePath(); cx.fill(); cx.restore(); }
  }

  // enemies
  for(const e of state.entities){ cx.save(); cx.translate(e.x,e.y); let color = e.etype==='boss'? '#f06c6c' : e.etype==='turret'? '#a48df2' : e.etype==='archer'||e.etype==='sniper'? '#87c1ff' : e.etype==='warlock'? '#ffb86b' : e.etype==='bomber'? '#ffd263' : '#c7d2e0'; if(e.elite){ color = '#ff6b81'; }
    cx.fillStyle=color; cx.beginPath(); cx.arc(0,0,e.r,0,TWO_PI); cx.fill(); const p=state.player; const a=Math.atan2(p.y-e.y,p.x-e.x); cx.fillStyle='#0b0f15'; cx.beginPath(); cx.arc(Math.cos(a)*6,Math.sin(a)*6,3.2,0,TWO_PI); cx.fill(); // hp ring
    cx.strokeStyle='#2b394d'; cx.lineWidth=4; cx.beginPath(); cx.arc(0,0,e.r+5,0,TWO_PI); cx.stroke(); const baseMax = (e.etype==='boss')?(80+state.floor*20): (e.etype==='turret'?(4+Math.floor(state.floor/1.5)):(e.etype==='archer'||e.etype==='sniper'?(3+Math.floor(state.floor/2)):(e.etype==='warlock'? (5+Math.floor(state.floor/2)) : (2+Math.floor(state.floor/2))))); const hpPct=clamp(e.hp/baseMax,0,1); cx.strokeStyle='#a1cdfc'; cx.beginPath(); cx.arc(0,0,e.r+5,-Math.PI/2,-Math.PI/2 + hpPct*TWO_PI); cx.stroke(); cx.restore(); }

  // enemy bullets
  cx.fillStyle = '#f6a38a'; for(const b of state.ebullets){ cx.beginPath(); cx.arc(b.x,b.y,b.r,0,TWO_PI); cx.fill(); }

  // player
  const p = state.player; if(p){ cx.save(); cx.translate(p.x,p.y); if(p.invuln>0){ cx.globalAlpha = 0.6 + 0.4*Math.sin(state.time*0.02); } cx.fillStyle = '#e8eef7'; cx.beginPath(); cx.arc(0,0,p.r,0,TWO_PI); cx.fill(); cx.strokeStyle = '#0b0f15'; cx.lineWidth = 3; cx.beginPath(); cx.arc(0,0,p.r-4, p.angle-0.4, p.angle+0.4); cx.stroke(); cx.save(); cx.rotate(p.angle); cx.fillStyle = '#a1cdfc'; cx.fillRect(8,-2, 14,4); cx.restore(); cx.restore(); }
...
function drawDoorHotspot(side){ /* visual only */ }

function tryChangeRoom(){
  if(state.lockDoors) return; if(!state.room || !state.map) return; const p=state.player; const x=ROOM_ORIGIN.x, y=ROOM_ORIGIN.y, w=ROOM_W, h=ROOM_H;
  const near = 28;
  if(state.room.doors.up && Math.abs(p.x-(x+w/2))<DOOR_W/2 && p.y<y+near){ const next = state.map.grid[state.room.y-1][state.room.x]; saveCurrentRoomState(); enterRoom(next); p.y = y+h-40; }
  else if(state.room.doors.down && Math.abs(p.x-(x+w/2))<DOOR_W/2 && p.y>y+h-near){ const next = state.map.grid[state.room.y+1][state.room.x]; saveCurrentRoomState(); enterRoom(next); p.y = y+40; }
  else if(state.room.doors.left && Math.abs(p.y-(y+h/2))<DOOR_W/2 && p.x<x+near){ const next = state.map.grid[state.room.y][state.room.x-1]; saveCurrentRoomState(); enterRoom(next); p.x = x+w-40; }
  else if(state.room.doors.right && Math.abs(p.y-(y+h/2))<DOOR_W/2 && p.x> x+w-near){ const next = state.map.grid[state.room.y][state.room.x+1]; saveCurrentRoomState(); enterRoom(next); p.x = x+40; }
}
...
function drawMiniMap(){
  ...
  // copy/keep helper functions from original file (lineCircle, outOfRoom, confineToRoom, drawRoomFrame, drawDoors, etc.)
  function explodeWrapper(x,y){ explode(state,x,y); } // convenience if other code calls explode

  function lineCircle(x1,y1,x2,y2,cx,cy,r){
    const dx=x2-x1, dy=y2-y1; const l2=dx*dx+dy*dy; if(l2===0) return false; let t=((cx-x1)*dx+(cy-y1)*dy)/l2; t=clamp(t,0,1); const px=x1+t*dx, py=y1+t*dy; const d2=dist2(px,py,cx,cy); return d2<=r*r;
  }

  function outOfRoom(x,y){ return x<ROOM_ORIGIN.x || x>ROOM_ORIGIN.x+ROOM_W || y<ROOM_ORIGIN.y || y>ROOM_ORIGIN.y+ROOM_H; }
  function confineToRoom(o){ o.x = clamp(o.x, ROOM_ORIGIN.x+20, ROOM_ORIGIN.x+ROOM_W-20); o.y = clamp(o.y, ROOM_ORIGIN.y+20, ROOM_ORIGIN.y+ROOM_H-20); }
  ...
  function loop(t){ const dt = clamp(t-last, 0, 40); last=t; if(!state.idle && !state.dead && !state.win){ try { update(dt); tryChangeRoom(); } catch(err){ console.error('Update error:', err); } } draw(); requestAnimationFrame(loop); }
