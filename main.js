...
(function(){
  ...
  function update(dt){
    ...
    for(let i=state.entities.length-1;i>=0;i--){ const e = state.entities[i]; e.t += dt;
      ...
      else if(e.etype==='turret'){ e.shootCD -= dt; if(e.shootCD<=0){ e.shootCD = 1200; const n=10; const base = state.time*0.004; for(let k=0;k<n;k++){ const a=base+(k/n)*TWO_PI; const sp=1.9; state.ebullets.push({x:e.x,y:e.y,r:4,vx:Math.cos(a)*sp, vy:Math.sin(a)*sp, life:3600, type:'slime'}); } } }
